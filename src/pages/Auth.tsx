...
const Auth = () => {
  ...
  // Parse OAuth redirect errors (?error=... or hash params)
  useEffect(() => {
    const extract = (s: string) => {
      const p = new URLSearchParams(s);
      return p.get('error_description') || p.get('error') || p.get('error_code');
    };
    const fromSearch = extract(window.location.search);
    const fromHash = window.location.hash.startsWith('#') ? extract(window.location.hash.slice(1)) : null;
    const err = fromSearch || fromHash;
    if (err) {
      toast({
